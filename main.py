...
# *****************************************************************************
# ##################NOTES TO SELF
# ADD TRIGGER CONFIG TO TEK SCOPE...
...
def create_test_directories(dcct_sn, dir_time_formatted):  # pylint: disable=redefined-outer-name
    ...
    report_path = os.path.join("Test_Data", f"DCCT_{dcct_sn}-{dir_time_formatted}", f"DCCT_{dcct_sn}_Report.pdf")
    raw_data_path = f"./Test_Data/DCCT_{dcct_sn}-{dir_time_formatted}/raw_data"  # pylint: disable=redefined-outer-name

    # Create parent directories for report and raw data paths (not including the file name)
...
def get_test_tech_info():
    ...
    while True:
        tester_name = input("Enter your name: ")  # pylint: disable=redefined-outer-name
        tester_life = input("Enter your Life #: ")  # pylint: disable=redefined-outer-name
...
def get_dcct_info():
    ...
    while True:
        dcct_sn_raw = input("Enter DCCT S/N: ")  # pylint: disable=redefined-outer-name
        dcct_sn = dcct_sn_raw[:13]
        if input(f"You entered: {dcct_sn}, is this correct? <Y/N>: ") in ("Y", "y"):
            ...
            file_path_l = os.path.join(raw_data_path, f"{dcct_sn}_raw_label.csv")
            data = [dcct_sn_raw]
            ...
            try:
                ...
                print(f"Error writing to {file_path_l}: {e}")
        return dcct_sn, dir_create_time, dir_time_formatted, report_date_formatted, \
...
def run_fault_test():
    ...
    while True:
        ...
        # **********************************************************************************
        # Save test data to file...
        file_path_l = os.path.join(raw_data_path, f"{dcct_sn}_fault_test_raw_data.csv")

        try:
            # Open the file in write mode (will create the file if it doesn't exist)
            # Open the file in write mode
            with open(file_path_l, mode='w', newline='') as file:
                writer = csv.writer(file)

                # Write the header (keys of the dictionary)
                writer.writerow(fault_test_results_l.keys())

                # Write the values (dictionary values in the same order as the keys)
        ...
        # n15v_voltage is the N15V rail PSU voltage at the time the FAULT flag asserted
        psu_init()  # Restore PSU voltages.
        print("Cycling DCCT PSU power...\n\n")
        sleep(3)

        if (fault_test_results_l["test_passed"] and fault_test_results_l["positive_assertion_test"] and
                fault_test_results_l["negative_assertion_test"] and fault_test_results_l["deassertion_test"]):
            print("All FLT12 Status tests passed. Proceeding to DCCT Current Testing...\n\n")
            return fault_test_results_l
        else:
            print("One or more FLT12 tests failed.")
            if input("Do you want to repeat FLT12 testing? <Y/N>: ") not in ("Y", "y"):
                if input("Do you want to restart with a new unit? <Y/N>: ") in ("Y", "y"):
                    return fault_test_results_l
                else:
                    if input("Proceed to DCCT Current Testing? <Y/N>: ") in ("Y", "y"):
                        return fault_test_results_l
...
def run_current_test():
    ...
    current_channel_data_l, current_decoded_wfdata_l = current_test(gen, psu, scope)
    ...
    file_path_chan_l = os.path.join(raw_data_path, f"{dcct_sn}_channel_data.csv")
    current_plot_filename_l = os.path.join(raw_data_path, f"waveform_plots_{dcct_sn}"
                                                          f"_{dir_time_formatted}.png")
    try:
        # Open the file in write mode and automatically close when the block is finished
        with open(file_path_chan_l, mode='w', newline='', encoding='utf-8') as file_chan_l:
            writer = csv.writer(file_chan_l)
            header = ["Channel", "Y Multiplier", "Y Zero", "Y Offset", "X Incrementation", "Waveform Data"]
            writer.writerow(header)

            # Write the channel data for each channel inside the open block
            for channel, data in current_channel_data_l.items():
                writer.writerow([channel, data["ymult"], data["yzero"], data["yoff"],
                                 data["xincr"], data["data"]])
    ...
    return current_channel_data_l, current_decoded_wfdata_l, current_plot_filename_l
# *************************************************************************
...
def generate_report_dataset():
    ...
    dut_info_l = {
        ...
        "flt12_test_passed_l": fault_test_results["test_passed"],  # noqa: E501  # pylint: disable = E0606
        "flt12_positive_assertion_test_passed_l": fault_test_results["positive_assertion_test"],  # noqa: E501  # pylint: disable = E0606
        "flt12_negative_assertion_test_passed_l": fault_test_results["negative_assertion_test"],  # noqa: E501  # pylint: disable = E0606
        "flt12_deassertion_test_passed_l": fault_test_results["deassertion_test"],  # noqa: E501  # pylint: disable = E0606
        "flt12_initial_voltage_l": fault_test_results["initial_voltage"],  # noqa: E501  # pylint: disable = E0606
        "flt12_positive_signal_voltage_l": fault_test_results["positive_assert_pin_voltage"],  # noqa: E501  # pylint: disable = E0606
        "flt12_negative_signal_voltage_l": fault_test_results["negative_assert_pin_voltage"],  # noqa: E501  # pylint: disable = E0606
        "flt12_positive_psu_voltage_l": fault_test_results["positive_assert_ps_voltage"],  # noqa: E501  # pylint: disable = E0606
        "flt12_negative_psu_voltage_l": fault_test_results["negative_assert_ps_voltage"],  # noqa: E501  # pylint: disable = E0606
...
while LOOP_FLAG == 1:
    ...
    current_channel_data, current_decoded_wfdata, current_plot_filename = run_current_test()  # pylint: disable=W0644
    current_plot_filename_l = os.path.join(raw_data_path, f"waveform_plots_{dcct_sn}"
                                           f"_{dir_time_formatted}.png")
    ...
    plot_pdf(dut_info, report_path, current_plot_filename)  # pylint: disable = E0606
